...
I will try all three methods and evaluate which normalization method separates African American samples and Caucasian samples by visualizing the separation on the PCA space.
...
```{r RUVg, fig.width=12}
set2 <- RUVr(set, rownames(set), k=8, res)
par(mfrow=c(1,2))
...
```{r volcano plot}
EnhancedVolcano(df.res.sym, x = "logFC", y = "PValue", lab = df.res.sym$SYMBOL,
                title = "Comparison: African American x Caucasian American",
...
```{r MA plot}
to.plot <- df.res.sym %>%
  dplyr::rename(log2FoldChange = logFC, baseMeanLog2 = logCPM, padj = PValue)

genenames <- df.res.sym$SYMBOL
...
ranked.gene.list <- lrt %>%
  as.data.frame() %>% 
  mutate(stat = logFC * -log10(PValue)) %>% ## here I rank by multiplying logFC by -log10(pvalue) to give more weight to more significant genes
...
```{r}
gseaplot2(gsea.go, geneSetID = 1:5, pvalue_table = TRUE)
```

```{r fig.height=8}
dotplot(gsea.go, showCategory = 30)
```

#### Enrichment analysis with filtered gene lists

```{r split gene lists into up and downregulated filtered by pvalue cutoffs}
p.05.up <- filter(df.res.sym, (PValue < 0.05) & (logFC > 0))
p.05.dn <- filter(df.res.sym, (PValue < 0.05) & (logFC < 0))

p.01.up <- filter(df.res.sym, (PValue < 0.01) & (logFC > 0))
p.01.dn <- filter(df.res.sym, (PValue < 0.01) & (logFC < 0))

p.001.up <- filter(df.res.sym, (PValue < 0.001) & (logFC > 0))
p.001.dn <- filter(df.res.sym, (PValue < 0.001) & (logFC < 0))

fdr.1.up <- filter(df.res.sym, (FDR < 0.1) & (logFC > 0))
fdr.1.dn <- filter(df.res.sym, (FDR < 0.1) & (logFC < 0))
```
...
```{r}
treeplot(ego.res$fdr.1.up)
```

```{r}
treeplot(ego.res$fdr.1.dn)
```
