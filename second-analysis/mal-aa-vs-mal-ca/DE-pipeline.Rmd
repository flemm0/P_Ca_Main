...
```{r}
knitr::opts_chunk$set(root.dir = "~/Desktop/velvilab/P_Ca_Analysis/second-analysis/mal-aa-vs-mal-ca")
```
...
```{r}
if (!file.exists("./gene-lists/all-genes.csv")){
  write.csv(df.res.sym, "./gene-lists/all-genes.csv")
}
```
...
p <- dotplot(gsea.go, split=".sign", showCategory = 20) + facet_grid(.~.sign) + labs(title = title, subtitle = "GSEA - GO:BP")
p
...
barplots <- list()
for (i in 1:8) {
  dotplots[[i]] <- dotplot(all.msig.res[[i]]) + labs(title = title, subtitle = comparisons[[i]])
  barplots[[i]] <- barplot(all.msig.res[[i]]) + labs(title = title, subtitle = comparisons[[i]])
}

p1 <- do.call(grid.arrange, dotplots)
p2 <- do.call(grid.arrange, barplots)

if (!file.exists("./plots/ORA_MSIG_dotplots.jpeg")) {
  ggsave(filename="./plots/ORA_MSIG_dotplots.jpeg", p1, dpi = 800, width = 30, height = 30, units = "in")
}

if (!file.exists("./plots/ORA_MSIG_barplots.jpeg")) {
  ggsave(filename="./plots/ORA_MSIG_barplots.jpeg", p2, dpi = 800, width = 30, height = 30, units = "in")
}
```
