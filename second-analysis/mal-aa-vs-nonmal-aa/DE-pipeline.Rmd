...
Malignant African American VS Nonmalignant African American

```{r}
knitr::opts_chunk$set(root.dir = "~/Desktop/velvilab/P_Ca_Analysis/second-analysis/mal-aa-vs-nonmal-aa")
```
...
gseaplot2(gsea.go, geneSetID = 1:8, pvalue_table = TRUE, title = paste(title, "Top 8 Pathways Identified by GSEA", sep = " "))
...
for (i in 1:length(indices)) {
    dotplots[[i]] <- dotplot(all.ego.res[[indices[i]]]) + labs(title = title, subtitle = comparisons[[indices[i]]])
    barplots[[i]] <- barplot(all.ego.res[[indices[i]]]) + labs(title = title, subtitle = comparisons[[indices[i]]])
}
