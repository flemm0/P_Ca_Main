...
```{r}
plotMD.DGEList(all.res$nb_mb)
```


```{r}
invisible(lapply(all.res, plotSmear, de.tags = "glmLRT", lowess = TRUE, main = "MA Plot"))
```
...
## Run DESeq and assign results to res
dds <- DESeq(dds)
res <- results(dds)
```


```{r convert ensembl id to gene symbol}
res.df <- as.data.frame(res)

# remove decimal points (version number) from ensembl ids, which causes error when converting to gene symbols
ensemblids <- sub("\\.[0-9]*$", "", rownames(res.df))
rownames(res.df) <- sub("\\.[0-9]*$", "", rownames(res.df))

geneids <- select(x = EnsDb.Hsapiens.v86,  key=ensemblids, columns=c("SYMBOL"), keytype="GENEID")
rownames(geneids) <- geneids$GENEID


# create column for gene symbol
res.df <- merge(x = res.df, y = geneids, by = "row.names", all = TRUE)

column_to_rownames(res.df, var = "Row.names")
