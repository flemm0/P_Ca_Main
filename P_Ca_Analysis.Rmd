...
```{r}
all.filtered.gene.lists <- list("p0.05" = genes.p05, "p0.01" = genes.p01, "p0.001" = genes.p001, "fdr0.1" = genes.fdr1)
...
invisible(lapply(names(to_write), function(name) {
  write.xlsx(to_write[[name]], file = paste0("./filtered_gene_lists/", name, ".xlsx"), rowNames = T)
  }
))
```
...
invisible(lapply(names(gsea.excel), function(x){ 
  write.xlsx(gsea.excel[[x]], 
